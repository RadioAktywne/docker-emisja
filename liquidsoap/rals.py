...
def get_credentials():
    ...
    """
    home_dir = os.path.expanduser('/home/liquidsoap/')
        ...
        for row in values:
            ...
            if ostatni!=row[kolumna]:
                if ostatni!='':
                    ...
                    slownik[ostatni] = czasformatls(poczatek) + ' - ' + czasformatls(zmianakonca(koniec))
...
def wpisyinputow(slur):
    lista=['#'+slur+'\n',
    'request_'+slur+' = request.equeue(id="request_'+slur+'")\n', 
    slur+' = fallback(track_sensitive=false, [request_'+slur+', studio, muzyka])\n']
    if ("grabek" not in slur) and ("plytatygodnia" not in slur) and ("gnp" not in slur):
        lista.append(slur+"_p = fallback(track_sensitive=false, [request_"+slur+", muzyka])\n")
    lista.append("#"+slur+"\n")
    return lista

def wpisynagrywania(slur):
    lista=['#'+slur+'\n', 'nagrywanie_'+slur+' = output.file(%vorbis,\n', '"/srv/ra/audycje/'+slur+'/powtorka/'+slur+'_#{time_stamp}.ogg",\n', 'fallible=true,\n', 'reopen_delay=128.0,\n', 'start=true\n', 'live_'+slur, ')\n', '#'+slur+'\n' ]
    return lista
        ...
        if 'Playlista' not in a:    
           # slur = list(a)
            ...
            if '_p' in a:
               # slur2 = a.replace("_p","")
               # slur2 = ''.join(slur)
                temp.append('({('+ dzien + 'w) and ' + slownik[a] + '}, ' + slur2 + '),\n')
            else:
                temp.append('({('+ dzien + 'w) and ' + slownik[a] + '}, ' + slur2 + '),\n')
...
def cronzeslownika(cron, slownik, dzien):
    ...
    for key in slownik.keys():
        ...
        if "_p" in key:
            ...
            job.setall(czas)
            #REQUEST POWTORKI
            comm = "python3 /home/liquidsoap/skrypty/requestpowtorki.py " + slur
            ...
            #czas = str(mp) + " " + str(hp) + " * * " + dzien
            #job.setall(czas)
            #comm = 'kill `ps -ef | grep "/etc/liquidsoap/' + slur + '_rec.liq" ' + "| grep -v grep | grep -v '/bin/sh' | awk '{print $2}'`"
            ...
            job.setall(czas)
            #PRZERZUT PUSZKI
            comm = "python3 /home/liquidsoap/skrypty/puszka_przerzut.py " + slur
...
#dorabia wpisy z listy slurow
    ...
    plik = open("/etc/liquidsoap/ra.liq",'r',encoding='utf-8')
    ...
    plik = open("/etc/liquidsoap/ra.liq",'w',encoding="utf-8")
    ...
    # for slur in slury:
    #     config = ['set("log.file.path","/var/log/liquidsoap/'+slur+'_rec.log")\n', "time_stamp = '%m-%d-%Y, %H:%M:%S'\n", 'studio = input.harbor("nagrywanie", user="source", password="hyrrdyrr", port=8006)\n','output.file(%vorbis, "/srv/ra/audycje/'+slur+'/powtorka/'+slur+'_#{time_stamp}.ogg", fallible=true, start=true, studio)\n' ]
